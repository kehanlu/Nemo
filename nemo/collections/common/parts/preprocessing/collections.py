...
class WhisperLlamaQAEntity(object):
    """
    kehan
    """
    def __init__(self, sid, audio_file, duration, question, transcription, answer, offset, speaker, orig_sr, lang) -> None:
        self.id = sid
        self.audio_file = audio_file
        self.duration = duration
        self.question = question
        self.transcription = transcription # kehan
        self.answer = answer
        self.offset = offset
        self.speaker = speaker
        self.orig_sr = orig_sr
...
class WhisperLlamaSpeechText(object):
    """
    kehan
    """
    ...
    def __init__(
        ...
        for id_, audio_file, duration, offset, question, transcription, answer, speaker, orig_sr, lang in zip(
            ids, audio_files, durations, offsets, questions, transcriptions, answers, speakers, orig_sampling_rates, langs
        ):
            # Duration filters.
            if duration is not None:
                curr_min_dur = min(duration) if isinstance(duration, list) else duration
            ...
            data.append(WhisperLlamaQAEntity(
                sid=id_, 
...
class WhisperLlamaCollection(WhisperLlamaSpeechText):
    """
    kehan
    audio, transcription, question, answer, durations, offsets
    """
    def __init__(
            self,
            manifests_files: Union[str, List[str]],
        ...
        ids, audio_files, transcriptions, durations, questions, answers, offsets, = (
                [],
                [],
                [],
                [],
        ...
        super().__init__(
            ids=ids, audio_files=audio_files, durations=durations, questions=questions, transcriptions=transcriptions, answers=answers, offsets=offsets, speakers=speakers, orig_sampling_rates=orig_srs, langs=langs, *args, **kwargs
        )
    ...
    def __parse_item(self, line: str, manifest_file: str) -> Dict[str, Any]:
        ...
        item = dict(
            ...
            duration=item['duration'],
            question=item['question'],
            transcription=item["transcription"], # kehan
            answer=item['answer'],
            offset=item.get('offset', None),
            speaker=item.get('speaker', None),
            orig_sr=item.get('orig_sample_rate', None),
